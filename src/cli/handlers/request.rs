	...
	pub async fn handle_request_command(
		...
		match &request_command.request_subcommand {
			...
			RequestSubcommand::Params { subcommand, .. } => {
				...
				let query_param_index =
					self.find_query_param(collection_index, request_index, key)?;

				match subcommand {
					KeyValueCommand::Key(key_command) => match key_command {
						KeyCommand::Get { .. } => self.cli_print_query_param(
							collection_index,
							request_index,
							query_param_index,
						),
						...
						KeyCommand::Delete { .. } => self.delete_query_param(
							collection_index,
							request_index,
							query_param_index,
						),
			...
			RequestSubcommand::Header { subcommand, .. } => {
				...
				match subcommand {
					...
					KeyValueCommand::Toggle { state, .. } => self.toggle_header(
						collection_index,
						request_index,
						state.clone(),
						header_index,
			...
			RequestSubcommand::Body { subcommand, .. } => match subcommand {
				...
				BodySubcommand::Key { subcommand } => {
					...
					let form_data_index =
						self.find_form_data(collection_index, request_index, key)?;
					...
					match subcommand {
						KeyValueCommand::Key(key_command) => match key_command {
							...
							KeyCommand::Delete { .. } => self.delete_form_data(
								collection_index,
								request_index,
								form_data_index,
							),
