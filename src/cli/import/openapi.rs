...
use anyhow::anyhow;
use openapiv3::{
	APIKeyLocation, OpenAPI, Operation, Parameter, ParameterSchemaOrContent, PathItem, ReferenceOr,
	RequestBody, Schema, SchemaKind, SecurityRequirement, SecurityScheme, Type,
};
...
pub fn process_path_operations(
	...
	if let Some(op) = &path_item.options {
		let name = op
			.operation_id
			.clone()
			.unwrap_or_else(|| format!("OPTIONS {}", path));
	...
	if let Some(op) = &path_item.head {
		let name = op
			.operation_id
			.clone()
			.unwrap_or_else(|| format!("HEAD {}", path));
	...
	for param_or_ref in &operation.parameters {
		...
		match param {
			Parameter::Query { parameter_data, .. } => {
				...
				request.params.push(KeyValue {
					...
					data: (
						...
						default_value.unwrap_or(String::from("value")),
					),
				});
			}
			Parameter::Header { parameter_data, .. } => {
				// Add header
				let default_value =
					extract_default_value(&parameter_data.format).unwrap_or(String::from("value"));
...
pub fn resolve_parameter_reference<'a>(
	param_or_ref: &'a ReferenceOr<Parameter>,
	spec: &'a OpenAPI,
) -> anyhow::Result<&'a Parameter> {
	match param_or_ref {
			...
			if let Some(components) = &spec.components {
				if let Some(param) = components.parameters.get(param_name) {
					match param {
						...
						ReferenceOr::Reference { .. } => Err(anyhow!(
...
pub fn extract_default_value(format: &ParameterSchemaOrContent) -> Option<String> {
	match format {
		ParameterSchemaOrContent::Schema(schema_or_ref) => {
			match schema_or_ref {
				ReferenceOr::Item(schema) => {
					// Extract default value from schema if available
					schema.schema_data.default.as_ref().map(|v| match v {
...
pub fn process_request_body(
	request: &mut Request,
	req_body_or_ref: &ReferenceOr<RequestBody>,
	spec: &OpenAPI,
) -> anyhow::Result<()> {
	...
	if let Some((content_type, media_type)) = req_body.content.iter().next() {
		...
		match content_type.as_str() {
			...
			"text/html" => {
				http_request.body =
					ContentType::Html("<html><body>Sample HTML</body></html>".to_string());
			}
			"application/javascript" => {
				http_request.body =
					ContentType::Javascript("console.log('Sample JavaScript');".to_string());
			}
			_ => {
				return Err(anyhow!(ImportOpenApiError::UnknownContentType(
					content_type.clone()
				)));
			}
		}
...
pub fn generate_sample_json(
	...
	serde_json::to_string_pretty(&sample_value)
		.map_err(|e| anyhow!("Failed to serialize sample JSON: {}", e))
}
