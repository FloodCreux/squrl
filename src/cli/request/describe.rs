...
use crate::app::key_value::print_key_value_vector;
...
impl App<'_> {
	pub fn cli_describe_request(
		...
		match (
			request.scripts.pre_request_script.is_some(),
			request.scripts.post_request_script.is_some(),
		) {
			(false, false) => {}
