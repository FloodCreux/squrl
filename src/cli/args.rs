...
pub struct Args {
	/// Main application directory, containing JSON/YAML collections files, the squrl.toml config file and the squrl.log file
	...
	match path_buf {
		...
		None => match env::var("SQURL_MAIN_DIR") {
			...
			Err(_) => match config_directory {
				...
				None => panic_error(
					"No directory provided, provide one either with `--directory <dir>` or via the environment variable `SQURL_MAIN_DIR`",
				),
			},
		},
	}
