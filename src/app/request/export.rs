...
impl App<'_> {
	...
	fn raw_html(
		...
		output += &match &http_request.body {
			...
			File(file_path) => {
				...
				let file_content =
					match get_file_content_with_name(PathBuf::from(&file_path_with_env_values)) {
	...
	fn php_guzzle(
		...
		if let Some(options) = options_str {
			...
			output += &format!(
				"\n$request = new GuzzleHttp\\Psr7\\Request('{}', '{}', $headers",
				http_request.method, url
			);
			if has_body {
				output += ", $body";
...
mod tests {
	...
	fn test_encode_basic_auth_special_chars() {
		let username = String::from("user@domain.com");
		let password = String::from("p@ss:word!");
		let result = encode_basic_auth(&username, &password);
		// "user@domain.com:p@ss:word!" base64 encoded
		assert!(result.starts_with("Basic "));
		// Verify it decodes back correctly
		let encoded_part = result.strip_prefix("Basic ").unwrap();
		let decoded = base64::prelude::BASE64_STANDARD
			.decode(encoded_part)
			.unwrap();
		assert_eq!(
			String::from_utf8(decoded).unwrap(),
			"user@domain.com:p@ss:word!"
		);
	}

	#[test]
	fn test_encode_basic_auth_unicode() {
		let username = String::from("usuario");
		let password = String::from("contraseña");
		let result = encode_basic_auth(&username, &password);
		assert!(result.starts_with("Basic "));
		// Verify it decodes back correctly
		let encoded_part = result.strip_prefix("Basic ").unwrap();
		let decoded = base64::prelude::BASE64_STANDARD
			.decode(encoded_part)
			.unwrap();
		assert_eq!(String::from_utf8(decoded).unwrap(), "usuario:contraseña");
	}

	#[test]
	fn test_escape_single_quote() {
		let result = escape("it's a test", '\'');
		assert_eq!(result, "it\\'s a test");
	}

	#[test]
	fn test_escape_double_quote() {
		let result = escape("say \"hello\"", '"');
		assert_eq!(result, "say \\\"hello\\\"");
	}

	#[test]
	fn test_escape_no_match() {
		let result = escape("no quotes here", '\'');
		assert_eq!(result, "no quotes here");
	}

	#[test]
	fn test_escape_multiple_occurrences() {
		let result = escape("'one' and 'two'", '\'');
	...
	#[test]
	fn test_escape_only_escape_char() {
		let result = escape("'''", '\'');
		assert_eq!(result, "\\'\\'\\'");
	}
}
