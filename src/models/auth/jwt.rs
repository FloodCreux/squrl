...
impl JwtAlgorithm {
	...
	pub fn get_helper(&self) -> &str {
		match self {
			...
			| JwtAlgorithm::PS256
			| JwtAlgorithm::PS384
			| JwtAlgorithm::PS512 => "RSA key file path",
	...
	let encoding_key = match algorithm {
		JwtAlgorithm::HS256 | JwtAlgorithm::HS384 | JwtAlgorithm::HS512 => match secret_type {
			...
			JwtSecretType::Base64 => {
				...
				EncodingKey::from_base64_secret(secret.as_str())
					.map_err(|_| JwtError::Base64DecodeError)?
			}
			JwtSecretType::UrlSafeBase64 => {
				trace!("Loading JWT URL safe Base64 secret");

				EncodingKey::from_urlsafe_base64_secret(secret.as_str())
					.map_err(|_| JwtError::Base64DecodeError)?
			}
			_ => unreachable!(),
		},
		JwtAlgorithm::ES256 | JwtAlgorithm::ES384 => {
			...
			match secret_type {
				JwtSecretType::Pem => {
					EncodingKey::from_ec_pem(&bytes).map_err(|_| JwtError::InvalidKeyFormat)?
				}
		...
		| JwtAlgorithm::PS512 => {
			...
			match secret_type {
				JwtSecretType::Pem => {
					EncodingKey::from_rsa_pem(&bytes).map_err(|_| JwtError::InvalidKeyFormat)?
				}
		...
		JwtAlgorithm::EdDSA => {
			...
			match secret_type {
				JwtSecretType::Pem => {
					EncodingKey::from_ed_pem(&bytes).map_err(|_| JwtError::InvalidKeyFormat)?
				}
