...
impl App<'_> {
	pub(super) fn render_request_script(&mut self, frame: &mut Frame, area: Rect) {
		...
		self.script_console.pre_request_text_area.display_cursor =
			display_pre_request_script_cursor;
		self.script_console.post_request_text_area.highlight_text = highlight_post_request_script;
		self.script_console.post_request_text_area.display_cursor =
			display_post_request_script_cursor;

		frame.render_widget(
			MultiLineTextInput(
				&mut self.script_console.pre_request_text_area,
				JS_SYNTAX_REF.clone(),
			),
			scripts_layout[0],
		);
		...
		frame.render_widget(
			MultiLineTextInput(
				&mut self.script_console.post_request_text_area,
				JS_SYNTAX_REF.clone(),
			),
			scripts_layout[2],
		);
