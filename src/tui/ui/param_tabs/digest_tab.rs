...
use crate::tui::app_states::AppState::{
	EditingRequestAuthDigestDomains, EditingRequestAuthDigestNonce, EditingRequestAuthDigestOpaque,
	EditingRequestAuthDigestPassword, EditingRequestAuthDigestRealm,
	EditingRequestAuthDigestUsername, SelectedRequest,
};
...
impl App<'_> {
	pub(super) fn render_digest_tab(&mut self, frame: &mut Frame, area: Rect) {
		...
		let mut stale_paragraph =
			Paragraph::new(digest_auth.stale.to_string()).fg(THEME.read().ui.font_color);
		let mut algorithm_paragraph =
			Paragraph::new(digest_auth.algorithm.to_string()).fg(THEME.read().ui.font_color);
		let mut qop_paragraph =
			Paragraph::new(digest_auth.qop.to_string()).fg(THEME.read().ui.font_color);
		let mut user_hash_paragraph =
			Paragraph::new(digest_auth.user_hash.to_string()).fg(THEME.read().ui.font_color);
		...
		match input_selected {
			...
			9 if should_color_blocks => {
				...
				user_hash_paragraph =
					user_hash_paragraph.fg(THEME.read().others.selection_highlight_color);
			}
			10 if should_color_blocks => {
				charset_block = charset_block.fg(THEME.read().others.selection_highlight_color);
				charset_paragraph =
					charset_paragraph.fg(THEME.read().others.selection_highlight_color);
			}
			_ => {}
		}
		...
		let scroll_offset = match input_selected {
			0 => 0,
			_ => ((input_selected as u16 + 1) * 3)
				.saturating_sub(area.height.saturating_sub(scroll_adjustment + 2)),
		};
