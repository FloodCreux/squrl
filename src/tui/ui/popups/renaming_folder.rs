...
impl App<'_> {
	pub fn render_renaming_folder_popup(&mut self, frame: &mut Frame) {
		...
		let renaming_folder_area = popup_block.inner(area);
		...
		frame.render_widget(
			SingleLineTextInput(&mut self.rename_collection_input),
			renaming_folder_area,
		);
	}
}
