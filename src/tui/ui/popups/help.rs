...
impl App<'_> {
	pub fn render_help_popup(&mut self, frame: &mut Frame) {
		...
		let title_paragraph =
			Paragraph::new(self.help_popup.selection.to_string().bold().underlined())
				.centered()
		...
		let protocol = match &self.collections_tree.selected {
			Some(selected_request_index) => {
				let local_selected_request =
					self.get_request_from_selection(selected_request_index);
		...
		let events = &self.help_popup.selection.get_available_events(
			self.request_view,
