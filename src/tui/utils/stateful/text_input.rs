...
use crate::tui::utils::syntax_highlighting::{
	ENV_VARIABLE_SYNTAX_REF, ENV_VARIABLE_SYNTAX_SET, SYNTAX_SET, SYNTAX_THEME, THEME_SET,
};
...
use edtui::{
	...
	LineNumbers, SyntaxHighlighter, system_editor,
};
...
impl<'a> Widget for SingleLineTextInput<'a> {
		...
		Self: Sized,
	{
		...
		let should_display_status = self.0.display_cursor
			...
			&& !(self.0.default_mode == EditorMode::Insert
				&& self.0.state.mode == EditorMode::Insert);
		...
		if should_display_status {
			...
			let status_line = Span::raw(self.0.state.mode.name()).style(
				Style::new()
					.fg(THEME.read().ui.font_color)
					.bg(status_line_bg_color),
			);
...
impl TextInput {
	...
	pub fn key_event(
		...
		if let Some(terminal) = terminal
			&& system_editor::is_pending(&self.state)
		{
	...
	match text_input.mode {
		TextAreaMode::Vim => {
			...
			if is_single_line {
				...
				handler.remove(&KeyEventRegister::n(vec![KeyInput::new(KeyCode::Char(
					'o',
				))]));
				handler.remove(&KeyEventRegister::n(vec![KeyInput::new(KeyCode::Char(
					'O',
				...
				handler.remove(&KeyEventRegister::n(vec![KeyInput::ctrl(KeyCode::Char(
					'e',
				))]));
			}
		...
		TextAreaMode::Emacs => {
			...
			if is_single_line {
				// Remove new line
				handler.remove(&KeyEventRegister::i(vec![KeyInput::ctrl(KeyCode::Char(
					'o',
				))]));
				handler.remove(&KeyEventRegister::i(vec![KeyInput::new(KeyCode::Enter)]));
				handler.remove(&KeyEventRegister::i(vec![KeyInput::ctrl(KeyCode::Char(
				...
				// External system editor
				handler.remove(&KeyEventRegister::i(vec![KeyInput::alt(KeyCode::Char(
					'e',
				))]))
			}
		...
		_ => {
			...
			let mut keys = HashMap::from([
				(
					KeyEventRegister::s(vec![KeyInput::from(delete_forward)]),
					RemoveCharFromSearch.into(),
				),
				// INSERT
				(
					KeyEventRegister::i(vec![KeyInput::from(move_cursor_right)]),
					MoveForward(1).into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(move_cursor_left)]),
					MoveBackward(1).into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(move_cursor_up)]),
					MoveUp(1).into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(move_cursor_down)]),
					MoveDown(1).into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(skip_word_right)]),
					MoveWordForward(1).into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(skip_word_left)]),
					MoveWordBackward(1).into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(move_cursor_line_start)]),
					MoveToStartOfLine().into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(move_cursor_line_end)]),
					MoveToEndOfLine().into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(delete_forward)]),
					DeleteChar(1).into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(delete_backward)]),
					RemoveChar(1).into(),
				),
				(KeyEventRegister::i(vec![KeyInput::from(undo)]), Undo.into()),
				(KeyEventRegister::i(vec![KeyInput::from(redo)]), Redo.into()),
				(
					KeyEventRegister::i(vec![KeyInput::from(copy)]),
					CopySelection.into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(paste)]),
					Paste.into(),
				),
				(
					KeyEventRegister::i(vec![KeyInput::from(system_editor)]),
			...
			if !is_single_line {
				keys.insert(
					KeyEventRegister::i(vec![KeyInput::from(new_line)]),
					LineBreak(1).into(),
				);
			}
			...
			if !insert_mode_only {
				keys.extend([
					...
					(
						KeyEventRegister::s(vec![KeyInput::from(move_cursor_down)]),
						FindNext.into(),
					),
					(
						KeyEventRegister::s(vec![KeyInput::from(move_cursor_up)]),
						FindPrevious.into(),
					),
					(
